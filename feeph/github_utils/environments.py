...
class Environments:
    ...
    def update_secret(self, environment: str, name: str, value: str) -> tuple[bool, str | None]:
        """
        update a GitHub environment secret

        https://github.com/<owner>/<repo>/settings/environments/<environmentid>/edit
        """
        ...
        response = self.session.put(f"{self.base_url}/environments/{environment}/secrets/{name}", json={'encrypted_value': encrypted.encrypted_value, 'key_id': encrypted.key_id})
